                            ...
                            int* values, int* solution)
{
    ...
    for (i=0; i<nz; i++) {
        ...
        if (TRACEPCP_QUERY_MULTIPLICATION_ENABLED())
          TRACEPCP_QUERY_MULTIPLICATION(solution[row_ind[i]], vector[col_ind[i]], values[i]);
    }
    end = omp_get_wtime();
    if (TRACEPCP_QUERY_COMPUTEMULTIPLICATIONTIME_ENABLED())
      TRACEPCP_QUERY_COMPUTEMULTIPLICATIONTIME((end - start) * 1000);
...
int main(int argc, char *argv[])
{
    ...
    if (TRACEPCP_QUERY_COUNTNONZEROS_ENABLED())
      TRACEPCP_QUERY_COUNTNONZEROS(nz);
    ...
    if (TRACEPCP_QUERY_CHECKSOLUTION_ENABLED())
      TRACEPCP_QUERY_CHECKSOLUTION(check); 
